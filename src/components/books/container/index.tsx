...
const BookContainer = ({content, title}: BookContainerProps) => {
    ...
    return (
        <>
            <Center>
                <Image src={content[currentPage].imageURL} rounded='xl' w={['80%', '100%']} h={['80%', '100%']} alt={title + '' + currentPage.toString()}></Image>
            </Center>
            <Center><Text w='100%' mt={6}>{content[currentPage].description}</Text></Center>
            <Center mt={6}>
                {currentPage > 0 && <IconButton onClick={() => setCurrentPage(prevPage => prevPage - 1)} aria-label='Next page' icon={<FaAngleLeft />} rounded='full' />}
